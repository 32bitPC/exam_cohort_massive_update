...
class cohort_upload_form extends moodleform {
    ...
    protected function process_upload_file($file, $encoding, $delimiter, $defaultcontext) {
        ...
        while ($row = $cir->next()) {
            ...
            $mark[$rownum] = 0;
            if (!empty($hash['idnumber'])) { //kt du lieu excel
                if ($DB->record_exists('cohort', array('idnumber' => $hash['idnumber']))) 
                {
                    $cohorts[$rownum]['warnings'][] = "MÃ£ lá»›p Ä‘Ã£ tá»“n táº¡i trong há»‡ thá»‘ng";
                    $mark[$rownum] = 1;
                    $cohort_exist[$rownum] = 1;
                }
if (isset($idnumbers[$hash['idnumber']])) 
{
                    $cohorts[$rownum]['warnings'][] = "MÃ£ lá»›p Ä‘Ã£ cÃ³ trong danh sÃ¡ch upload";
                    $mark[$rownum] = 1;
                    $cohort_exist[$rownum] = 1;
                }
            ...
            {
              $cohorts[$rownum]['errors'][] = "MÃ£ ká»³ thi Ä‘ang Ä‘á»ƒ trá»‘ng<br>";
            }
            if (empty($hash['name'])) {
            ...
            //2
            $hash['idnumber'] = ltrim($hash['idnumber']);
...
";
            ...
            if (empty($check_course)) {
                $cohorts[$rownum]['errors'][] ="Nháº­p sa chÆ°Æ¡ng trÃ¬nh: $chuongtrinh<br>";
            }
            //3
            $cohort_idnumber = $hash['idnumber'];
            if($mark[$rownum] == 1)
            {
              $err = "
          select id from mdl232x0_cohort where idnumber = N'$cohort_idnumber' and courseid = $check_course";
              $array_err = $DB->get_records_sql($err,array());
              foreach($array_err as $ae){
                  $check_course_cohort= $ae->id;
              }
              if (empty($check_course_cohort)) {
                  $cohorts[$rownum]['errors'][] ="KhÃ³a há»c chÆ°a tá»“n táº¡i trong há»‡ thá»‘ng hoáº·c chÆ°a assign cho mÃ£ lá»›p<br>";
              }
            }

            //

            $kiemtramakythi = $hash['testcode'];

            $array_sqlr = "
        select id from mdl232x0_cohort where idnumber = N'$cohort_idnumber'
";
            $arrayr = $DB->get_records_sql($array_sqlr,array());
            foreach($arrayr as $az){
                $check_cohort[$rownum]= $az->id;
            }

            $array_sql = "
        select id from mdl232x0_cohort where testcode = N'$kiemtramakythi' and idnumber = N'$cohort_idnumber'
";
            $array = $DB->get_records_sql($array_sql,array());
            foreach($array as $a){
                $check_examcode[$rownum]= $a->id;
            }
            //custom
...
";
            ...
            //
            if (($hash['online']==0 || empty($hash['online'])) && ($hash['address_training'] == '' || $hash['address_test'] == ''))
            {
                $cohorts[$rownum]['errors'][] ="Lá»›p offline nhÆ°ng field Ä‘á»‹a Ä‘iá»ƒm bá»‹ trá»‘ng<br>";
            }
