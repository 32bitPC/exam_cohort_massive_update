...
$cma3,$cma4,$cma5,$cma6)
{
  ...
  $makythi->id = $DB->insert_record('cohort_examcode',$makythi); // add new makythi row
  $reupdate_sql ="
  update mdl232x0_cohort_examcode
...
function add_examcode_enrol($cohortid,$enrolid,$examcode)
{
  global $DB;
  $enrol_arr = "
update mdl232x0_enrol 
set examcode = N'$examcode'
where customint1 = $cohortid
and id = $enrolid
  ";
  $DB->execute($enrol_arr,array());
  $a = 1;
  return $a;
}
...
function check_examcode($cohortid,$examcode)
{
    ...
    select * from {cohort_examcode} where examcode = ? and cohortid = ?',array('examcode'=>$examcode,'cohortid'=>$cohortid));
    foreach($examcode_arr as $ea)
    {
      $check_examcode = $ea->id;
    }

    return $check_examcode;
}
//
function cohort_upload_add_cohort_debut($cohort) {
    ...
    if(is_null($cohort_id)) // check if this cohort is a new one
    {
        ...
        if(is_null($is_cohort_enrol) && $cohort->online == 1)
        {
          ...
          //  $update_this = add_examcode_enrol($cohort->id,$enrol->id,$cma1);
        }
...
// xet chuoi
    ...
    else
    {
      ...
      from mdl232x0_cohort_examcode
      where examcode = N'$cma1' and cohortid = $cohort->id)
";
